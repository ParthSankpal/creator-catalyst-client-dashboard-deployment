    ...
    <article className={`bg-white dark:bg-[#222222] rounded-xl shadow-xl p-6 border-l-3 ${unlocked ? (inProgress ? "border-emerald-500" : "border-emerald-600") : "border-gray-300"} flex flex-col`}>
      ...
      <div className="flex flex-col items-center justify-between mt-4 text-sm text-gray-600 dark:text-gray-400">
        <div className="flex justify-between w-full">
          Videos <span className="font-medium">{videos}</span>
        </div>
        <div className="flex justify-between w-full mt-2">
          Assignments <span className="font-medium">{assignments}</span>
        </div>
        {score !== undefined && (
          <div className="flex justify-between w-full mt-2">
        ...
        {!unlocked && (
          <div className="flex justify-between w-full mt-2">
            Unlocks At <span className="font-medium">{unlocksAt}</span>
          </div>
        )}
      </div>
